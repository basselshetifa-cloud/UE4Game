...
void main()
{
	...
	highp vec2 v1;
	highp vec2 v2;
	v2.xy = (gl_FragCoord.xy*View_BufferSizeAndInvSize.zw);
	v1.xy = v2;
	v1.x = clamp(v2.x,pu_h[1].x,pu_h[1].z);
	v1.y = clamp(v2.y,pu_h[1].y,pu_h[1].w);
	highp vec4 v3;
	v3.xyzw = texture(ps2,(gl_FragCoord.xy*pu_h[0].zw));
	highp vec4 v4;
	v4.xyzw = texture(ps1,v1);
	highp vec4 v5;
	v5.xyzw = texture(ps0,v1);
	highp vec2 v6;
	highp float f7;
	f7 = min(1.000000e+00,v3.w);
	if ((f7>5.000000e-01))
	{
		v6.x = ((f7+-5.000000e-01)*2.000000e+00);
		v6.y = 0.000000e+00;
	}
	else
	{
		v6.y = (1.000000e+00+(-(f7*2.000000e+00)));
		v6.x = 0.000000e+00;
	}
	highp float f8;
	f8 = sqrt(sqrt(v4.w));
	highp float f9;
	f9 = clamp(((2.500000e-01+(-(v6*vec2(View_DepthOfFieldScale)).x))*4.000000e+00),0.000000e+00,1.000000e+00);
	v0.xyz = mix(mix(mix(v3.xyz,(((v3.xyz*vec3(1.000000e-04,1.000000e-04,1.000000e-04))+v4.xyz)/vec3((v4.w+1.000000e-04))),vec3(((f8*f8)*(3.000000e+00+(-(2.000000e+00*f8)))))),v3.xyz,vec3((f9*f9))),(((v3.xyz*vec3(1.000000e-04,1.000000e-04,1.000000e-04))+v5.xyz)/vec3((v5.w+1.000000e-04))),vec3(clamp((v5.w*5.000000e+00),0.000000e+00,1.000000e+00)));
	v0.w = 0.000000e+00;
	out_Target0.xyzw = v0;
}
